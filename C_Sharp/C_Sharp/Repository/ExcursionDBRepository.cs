...
namespace C_Sharp.Repository
{
    public class ExcursionDBRepository : IExcursionRepository
    {
            ...
            using (var command = con.CreateCommand())
            {
                ...
                using (var dataReader = command.ExecuteReader())
                {
                    while (dataReader.Read())
                    {
                        ...
                        DateTime time = DateTime.Parse(dataReader.GetString(3));
        ...
        public IEnumerable<Excursion> FilterByObjective(string objectiveSearch, DateTime from, DateTime to)
        {
            IDbConnection con = JdbcManager.GetConnection();
            IList<Excursion> excursions = new List<Excursion>();
